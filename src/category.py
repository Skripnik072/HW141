...
class Category:
        ...
        if isinstance(product, Product):
            try:
                if product.quantity == 0:
                    raise ZeroQuantityProduct("Нельзя ввести продукт с нулевым количеством")
                ...
                print("Задача добавления продукта завершена")
    ...
    def middle_product_price(self):
        try:
            return sum([float(prod.gprice) for prod in self.__products]) / len(self.__products)
...
if __name__ == "__main__":
    ...
    print(cat1.middle_product_price())

    cat3 = Category("Fruit", "Japan", [])

    print(cat3.middle_product_price())
